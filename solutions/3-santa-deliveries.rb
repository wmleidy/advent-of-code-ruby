...
def robo_deliveries(input)
  ...
  input.chars.each_with_index do |char, i|
    if i % 2 == 0
      ...
      unless visited.index([santa_x, santa_y])
      ...
      unless visited.index([robo_x, robo_y])
        visited << [robo_x, robo_y]
        total += 1
      end
    end
  end
...
p deliveries(input)
p robo_deliveries(input)

### Other Approaches ###
# 1) use a visited hash instead of a visited array to speed up search for match
# 2) for Part Two, set up two "Santas" in an array and cycle through them, to DRY out the code, e.g. santas = [[0,0],[0,0]].cycle 