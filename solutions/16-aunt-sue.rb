...
# horrible, horrible code, took 75 minutes, placed #285
...
# Best not to look at this, plus I had to modify source file with spaces...
# def sue_io
#   sues = []

#   IO.foreach("help16-aunt-sue.txt") do |line|
#     number = line.split[1][0..-2].to_i
#     rest = line[9..-2]
#     h = {}
#     thing1 = rest.scan(/(\w+):/)[0][0]
#     thing2 = rest.scan(/(\w+):/)[1][0]
#     thing3 = rest.scan(/(\w+):/)[2][0]
#     quant1 = rest.scan(/(\d+)/)[0][0].to_i
#     quant2 = rest.scan(/(\d+)/)[1][0].to_i
#     quant3 = rest.scan(/(\d+)/)[2][0].to_i
#     h[thing1] = quant1
#     h[thing2] = quant2
#     h[thing3] = quant3
#     s = Sue.new(number, h)
...
def find_matching_sue(arr)
  ...
  arr.each do |sue|
    ...
    conditions.each do |k, v|
      if v != sue.send(k)
        flag = true unless sue.send(k) == "unknown"
      end
    end
    return sue unless flag
  end

  # Part Two
  # arr.each do |sue|
  #   if (sue.children == "unknown" || conditions[:children] == sue.children) &&
...
# My own refactor with Regex help from reddit
def sue_io
  sues = []
